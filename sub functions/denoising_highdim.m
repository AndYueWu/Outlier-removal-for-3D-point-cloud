function [x_reconstructed, o_reconstructed] = denoising_highdim(beta, y, max_iter, g, rho, alpha, tol, dim)
    ...
    L = [diag(ones(total_elements-1, 1)), zeros(total_elements-1, 1)] - [zeros(total_elements-1, 1), diag(ones(total_elements-1, 1))]; % Change to spdiag if algorithm works
    ...
    while k <= max_iter && norm(PG, inf) > tol 
        ...
        gradient = [GX; PG];
        Hessian = [2 * eye(total_elements) + 2 * alpha * L' * L, 2 * eye(total_elements); 2 * eye(total_elements), 2 * eye(total_elements) + beta * Gamma];
